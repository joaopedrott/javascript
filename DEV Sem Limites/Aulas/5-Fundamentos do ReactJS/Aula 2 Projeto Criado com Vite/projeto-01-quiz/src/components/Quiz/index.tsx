...
const QUESTIONS: Question[] = [
  //preciso tipar tambem a array de question
  {
    id: 1,
...
export function Quiz() {
  ...
  const handleAnswerQuestion = (
    ...
    const isCorretAnser =
      ...
      answer; /* se a resposta for igual a respostaCorreta= True */
  ...
  const handleNextQuestion = () => {
    //Funcao mudanca de perguntas
    if (currentQuestionNumber < quizSize) {
      //avanca para a proxima pergunta
      setCurrentQuestionIndex((index) => index + 1);
      setIsCurrentQuestionCorrect(false); //reseta para pergunta nao respondida
    } else {
  ...
  const navigationButtonText =
    currentQuestionNumber === quizSize ? "Ver Resultado" : "Proxima Pergunta";
  return (
        ...
        {isTakingQuiz ? (
          <div className={S.quiz}>
            ...
            <header className={S.quizHeader}>
              ...
              <h1 className={S.question}>{currentQuestion.question}</h1>{" "}
              {/* Nome da pergunta */}
            </header>

            <ul className={S.answers}>
              {currentQuestion.answers.map(
                (answer /* possiveis respostas da pergunta */) => (
                  <li key={answer} className={S.answerItem}>
                    ...
                    <QuestionAnswer
                      ...
                      disabled={isCurrentQuestionAswerd}
          ...
          <Result
            correctAnsersCount={correctAnsersCount}
            quizSize={quizSize}
